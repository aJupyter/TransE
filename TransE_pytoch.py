...
def dataloader(file1, file2, file3, file4):
    ...
    '''N18/wordnet-mlj12-train.txt 内容
...
class E(nn.Module):
    def __init__(self, entity_num, relation_num, dim, margin, norm, C):
        ...
        self.C = C
        '''
        torch.nn.Embedding 是 PyTorch 中用于实现嵌入层的类。它接受两个参数：
        num_embeddings：嵌入层中唯一标记的数量，也就是词汇表的大小。
    ...
    '''
    这个函数被用来计算嵌入的范数，并将其与1进行比较，以确保嵌入的范数不会超过1。
    如果嵌入的范数小于等于1，则损失为0。
    如果嵌入的范数大于1，则损失为嵌入的范数减去1。
    ...
    def forward(self, current_triples, corrupted_triples):
        ...
        x = torch.randn(3, 6)
        # 沿着维度 1 将张量分成 2 块
        chunks = torch.chunk(x, 2, dim=1) # (3, 3)
        '''
